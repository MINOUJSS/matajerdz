<li class="nav-item dropdown">
    <a class="nav-link dropdown-toggle waves-effect waves-dark pro-pic" href="" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        ...
        <span class="m-l-5 font-medium d-none d-sm-inline-block">{{Auth::guard('dropshiper')->user()->name}} <i class="mdi mdi-chevron-down"></i></span>
    </a>
    <div class="dropdown-menu dropdown-menu-right user-dd animated flipInY">
        ...
        <div class="d-flex no-block align-items-center p-15 bg-primary text-white m-b-10">
            ...
            <div class="m-l-10">
                <h4 class="m-b-0">{{Auth::guard('dropshiper')->user()->name}}</h4>
                <p class=" m-b-0">{{Auth::guard('dropshiper')->user()->email}}</p>
            </div>
        </div>
        <div class="profile-dis scrollable">
            ...
            <div class="dropdown-divider"></div>
            {{-- <a class="dropdown-item" href="javascript:void(0)"> --}}
                <a class="dropdown-item" href="{{route('dropshiper.logout')}}" onclick="event.preventDefault();
                document.getElementById('logout-form').submit();">
                <i class="fa fa-power-off m-r-5 m-l-5"></i> تسجيل الخروج</a>
                <form id="logout-form" action="{{route('dropshiper.logout')}}" method="post">
                @csrf
                </form>
            <div class="dropdown-divider"></div>
        </div>
