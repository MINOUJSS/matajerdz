...
@section('content')
       ...
       <!-- Login box.scss -->
       <!-- ============================================================== -->
       <div class="auth-wrapper d-flex no-block justify-content-center align-items-center" style="background:url({{asset('admins/'.$template)}}/assets/images/background/supplier-login-register.jpg) no-repeat;background-size: 70%;">
        <div class="auth-box on-sidebar">
            <div>
                <div class="logo">
                    <span class="db"><img src="{{asset('admins/'.$template)}}/assets/images/logo-icon.png" alt="logo" /></span>
                    <h5 class="font-medium m-b-20">تسجيل حساب المورد </h5>
                </div>
                <!-- Form -->
                <div class="row">
                    <div class="col-12">
                        <form method="POST" class="form-horizontal m-t-20" action="{{ route('supplier.register') }}">
                            @csrf
                            ...
                            <div class="form-group row">
                                <div class="col-12 ">
                                    <input class="form-control form-control-lg @error('store_name') is-invalid @enderror" type="text" placeholder="إسم متجرك يجب أن يكون فريداً و مميز" name="store_name" value="{{ old('store_name') }}" required autocomplete="store_name">
                                    @error('store_name')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="form-group row">
                                <div class="col-12 ">
                                    <select class="form-control form-control-lg @error('type') is-invalid @enderror" name="type">    
                                        <option value="تاجر جملة" @if(old('type')=='تاجر جملة'){{'selected'}}@endif>تاجر جملة</option>
                                        <option value="صاحب مصنع" @if(old('type')=='صاحب مصنع'){{'selected'}}@endif>صاحب مصنع</option>
                                        <option value="مستورد" @if(old('type')=='مستورد'){{'selected'}}@endif>مستورد</option>                                                                            
                                    </select>
                                    @error('type')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                            ...
                            <div class="form-group m-b-0 m-t-10 ">
                                <div class="col-sm-12 text-center ">
                                    هل تملك حساب على الموقع? <a href="{{route('supplier.login')}}" class="text-info m-l-5 "><b>تسجيل الدخول</b></a>
                                </div>
                            </div>
                        </form>
                    </div>
